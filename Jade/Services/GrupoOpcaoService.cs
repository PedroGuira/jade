...
namespace Jade.Services
{
    public class GrupoOpcaoService : IGrupoOpcaoService
    {
        ...
        public async Task<IEnumerable<GrupoOpcaoVerDto>> ObterTodosParaLojaDeFranquiaAsync(int estabelecimentoId)
        {
            ...
            if (loja == null) return new List<GrupoOpcaoVerDto>();

            var resultadoFinal = new List<GrupoOpcaoVerDto>();
            var gruposLocais = await ObterTodosDeLojaAsync(estabelecimentoId);
            resultadoFinal.AddRange(gruposLocais);

            if (loja.MarcaFranquiaId.HasValue)
            {
                var gruposTemplate = await ObterTodosTemplatesDaMarcaAsync(loja.MarcaFranquiaId.Value);
                // Adiciona apenas templates que ainda não existem como "locais" (baseado no nome, por exemplo, ou precisa de lógica mais complexa)
                // Por enquanto, apenas adiciona todos os templates da marca. O frontend pode lidar com a exibição.
                resultadoFinal.AddRange(gruposTemplate.Where(gt => !resultadoFinal.Any(gl => gl.Nome == gt.Nome && gl.EstabelecimentoId == estabelecimentoId)));
            }
            return resultadoFinal.OrderBy(g => g.IsTemplateDaFranquia).ThenBy(g => g.OrdemExibicao).ThenBy(g => g.Nome).ToList();
        }
