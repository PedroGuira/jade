    ...
    public class AuthController : ControllerBase
    {
        ...
        public async Task<ActionResult<LoginResponseDto>> Login(LoginDto loginDto)
        {
            ...
            var authClaims = new List<Claim>
            {
                ...
                new Claim("roleInternal", user.Role) // Sua claim de role interna
            };

            // Adiciona estabelecimentoId ou marcaFranquiaIdVinculada como claim se existirem
            if (user.EstabelecimentoId.HasValue) // user.EstabelecimentoId é int? (anulável)
            {
                // >>> PONTO CRÍTICO AQUI <<<
                authClaims.Add(new Claim("estabelecimentoId", user.EstabelecimentoId.Value.ToString()));
            }
            if (user.MarcaFranquiaIdVinculada.HasValue) // user.MarcaFranquiaIdVinculada é int? (anulável)
            {
            ...
            return Ok(new LoginResponseDto
            {
                ...
                Role = user.Role,
                EstabelecimentoId = user.EstabelecimentoId, // Para o DTO de resposta
                MarcaFranquiaId = user.MarcaFranquiaIdVinculada // Para o DTO de resposta
            });
        }
